...
import { Search, Grid, List, Filter, X, ChevronLeft, ChevronRight, MapPin, Calendar, DollarSign, Clock, Upload, CheckCircle, Phone } from 'lucide-react';
            ...
            <div className={`grid gap-6 ${viewMode === 'grid' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
              {filteredCategories.map((category) => {
                ...
                return (
                  <div
                    key={category._id}
                    className="bg-white rounded-xl p-8 shadow-sm border border-gray-200 hover:shadow-lg hover:-translate-y-1 transition-all duration-200 group cursor-pointer"
                    ...
                    role="button"
                    tabIndex={0}
                    aria-label={`Create request for ${category.name}`}
                  >
                    <div className="flex items-start space-x-4">
                      <div className={`inline-flex p-4 rounded-xl bg-gradient-to-r ${gradientClasses} mb-4 group-hover:scale-110 transition-transform duration-300`}>
